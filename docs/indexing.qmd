...
PRQL 0.7 does not support `` from `data/foo.csv` ``,
so we need to write raw SQL in [S-strings](https://prql-lang.org/book/language-features/s-strings.html) now.
([PRQL/prql#1535](https://github.com/PRQL/prql/issues/1535))
...
`or` operator will be changed to `||` PRQL >= 0.8.
...
```{prql}
from tab
filter IATA_CODE_Reporting_Airline == 'AA' or IATA_CODE_Reporting_Airline == 'DL'
take 5
```
