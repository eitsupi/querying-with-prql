...
parquet_path <- file.path(data_dir, "data.parquet")
...
#| cache: false
#| warning: false
CREATE TABLE tab AS SELECT * FROM 'data/data.parquet'
```
...
df <- duckdb::sql("FROM 'data/data.parquet'")
```
...
lf = pl.scan_parquet("data/data.parquet")
```
