...
and `groupby_dynamic` in Polars have completely different initial starting points by default.
...
- `groupby_dynamic` of Polars, the `offset` parameter to specify the origin point, is negative `every` by default.[^groupby_dynamic]
...
```{python}
lf.collect().upsample("time", every="6h").head(5)
```
...
### Moving Average, Cumurative Avarage {#sec-moving-ave}
...
In R, base R have some window functions like `cumsum`, but none like cumurative avarage.
...
```{prql}
func timezone tz col -> s"timezone({tz}, {col})"
